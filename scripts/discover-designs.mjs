...
function expectedSource(keys) {
  ...
  // The host default Design: Civic when present (it is the production oracle's
  // default policy), otherwise the first discovered Design so a host without
  // Civic still resolves unknown keys deterministically. ONE generated source
  // of truth — host code must never re-encode this policy as a literal.
  const defaultKey = keys.includes('civic') ? 'civic' : keys[0]
  return `// AUTO-GENERATED by scripts/discover-designs.mjs. Do not edit.\n\nexport const DEFAULT_DESIGN_KEY = '${defaultKey}' as const\n\nexport const DESIGN_KEYS = [\n${values}\n] as const\n\nexport type DesignKey = (typeof DESIGN_KEYS)[number]\n\nexport function isDesignKey(value: unknown): value is DesignKey {\n  return typeof value === 'string' && (DESIGN_KEYS as readonly string[]).includes(value)\n}\n`
}
