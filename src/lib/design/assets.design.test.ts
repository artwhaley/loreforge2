...
describe('bundled Design asset contract', () => {
  ...
  it('accepts media overrides and same-Design bundled refs only', () => {
    ...
    expect(isSanctionedDesignAssetUrl('/design-assets/ledger/assets/thumbnail.svg', 'obsidian')).toBe(false)
    ...
    expect(isDesignAssetRefForKey({ url: '/design-assets/ledger/assets/thumbnail.svg' }, 'obsidian')).toBe(false)
  })
})
