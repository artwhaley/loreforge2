...
/**
 * Records page model assembly (Stage F). Moved, not rewritten, from the
 * records route: the same request-owned session, two-axis predicate,
 * supersession closure, folder projection, and prepared-by aggregation.
 * Designs receive the identical authorized DTOs the explorer consumed.
 */
export async function buildRecordsPageModel(input: {
  ...
  const { tenant, user, activeCharacter, folderRaw, searchRaw } = input
  ...
  // The initial records are a lightweight 50-row window; nonblank searches use
  // the complete authorized server query and never ship document bodies.
  ...
  const toFolderSummary = (node: ProjectedFolder): FolderSummary => ({
    id: node.id,
    name: node.name,
    systemManaged: node.systemManaged,
    readableRecordCount: node.recordCount,
    children: node.children.map(toFolderSummary),
  })
  ...
  return {
    ...
    folders: projection.tree.map(toFolderSummary),
    totalReadableRecordCount: projection.totalReadable,
    records: allDocs.map(toRecordSummary),
    documentTypes: documentTypes.docs.map((type) => ({ id: Number(type.id), name: type.name })),
    supersessionEdges,
    query: { folderId: initialFolderId, search: typeof searchRaw === 'string' ? searchRaw.trim() : '' },
    capabilities: { manageFolders: canManageFolders, actOnRecords: Boolean(user), deleteRecords: canDeleteRecords },
