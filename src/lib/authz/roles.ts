...
export async function getHeldRoles(payload: Payload, args: { domainId: number | string; characterId: number | string }): Promise<RoleNode[]> {
  ...
  const assignments = await payload.find({ collection: 'role-assignments', where: { and: [{ character: { equals: args.characterId } }, { status: { equals: 'active' } }] }, depth: 0, limit: 10000, overrideAccess: true })
...
export async function getRoleDefaultRules(payload: Payload, args: { domainId: number | string; roleId: number | string }) {
  return payload.find({ collection: 'permission-rules', where: { and: [{ domain: { equals: args.domainId } }, { principalType: { equals: 'Role' } }, { principal: { equals: args.roleId } }, { active: { equals: true } }] }, depth: 1, limit: 10000, overrideAccess: true })
}
