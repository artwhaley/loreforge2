...
export function RecordsExplorer({ base, tenantSlug, folders, records, documentTypes, supersessionEdges, initialFolderId, initialSearch, canManageFolders, canActOnRecords, canDeleteRecords }: Props) {
  ...
  const contextRecordIsSuperseded = contextRecord ? supersessionEdges.some((edge) => edge.olderId === contextRecord.id) : false
  ...
  const canOfferSupersede = (record: ExplorerRecord | null | undefined) => Boolean(record && canSupersedeDocument(record.lifecycle))
