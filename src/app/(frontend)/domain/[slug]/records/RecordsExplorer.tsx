...
export function RecordsExplorer({ base, tenantSlug, folders, records, totalRecordCount, documentTypes, supersessionEdges, initialFolderId, initialSearch, canManageFolders, canActOnRecords, canDeleteRecords, vocabulary }: Props) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        const data = await response.json() as { results: Array<{ id: number; title: string; folderId: number | null; documentTypeId: number | null; updatedAt: string; preparedBy: string | null; lifecycle: string; locked: boolean }>; supersessionEdges?: SupersessionEdge[]; nextCursor?: string | null }
  ...
  // P07X-T04: folder badges use the server-filtered readable counts from the
  // projection; only the client-side type-exposure filter recounts locally.
  const directCountByFolder = useMemo(() => {
    const counts = new Map<number, number>()
    const visit = (nodes: ExplorerFolder[]) => nodes.forEach((folder) => { counts.set(folder.id, folder.recordCount); visit(folder.children) })
    visit(folders)
  ...
  // records (Filed / Deprecated). Drafts and Submitted records are
  ...
  const canOfferEdit = (record: ExplorerRecord | null) => Boolean(record?.canEdit && canEditDocumentBody(record.lifecycle as Lifecycle, Boolean(record.locked)))
