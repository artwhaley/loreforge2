...
export default async function DepartmentsPage({ params }: Props) {
  ...
  return <TenantShell tenant={tenant} cssVars={themeTokensToCssVars(resolveThemeTokens(tenant))} role={role} switcherTenants={domains}>
    <section><h1 className={styles.pageTitle}>{vocab.subdomain.plural}</h1><p className={styles.pageIntro}>The offices and working groups inside {tenant.name}. Choose a {vocab.subdomain.singular} to see its people and archive branches.</p>
      {role === 'admin' ? <p><a className={styles.manageLink} href={`/domain/${slug}/manage/people`}>Manage People</a></p> : null}
      {departments.length === 0 ? <p className={styles.empty}>No {vocab.subdomain.plural} have been configured.</p> : <ul className={styles.grid}>{departments.map((department) => <li key={department.id} className={styles.card}><h2 className={styles.cardTitle}><a href={`/domain/${slug}/departments/${department.slug}`}>{department.name}</a></h2><p className={styles.cardDesc}>{department.description || `A ${vocab.subdomain.singular} within this Domain.`}</p><p className={styles.cardMeta}>{memberCounts.get(Number(department.id)) ?? 0} active participant{memberCounts.get(Number(department.id)) === 1 ? '' : 's'}</p></li>)}</ul>}
    </section>
  </TenantShell>
}
