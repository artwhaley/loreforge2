...
export default async function TemplatesPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const canManageTemplates = await isAllowed({ payload, actor: { userId: user.id, activeCharacterId: activeCharacter?.id ?? null }, domainId: tenant.id, capability: 'manage_templates', resource: { type: 'Domain', id: tenant.id } })
  ...
  return <TenantShell tenant={tenant} cssVars={themeTokensToCssVars(resolveThemeTokens(tenant))} role={role} switcherTenants={domains} activeCharacter={activeCharacter}>
    <section style={{ maxWidth: 1100, margin: '0 auto', display: 'grid', gap: '1rem' }}>
      <nav aria-label="Document Types"><Link href={`/domain/${slug}/document-types`} title="Document Types are the first-order item; Templates and Forms hang off them">Document Types</Link> · <Link href={`/domain/${slug}/templates`} aria-current="page" title="Standalone Markdown templates">Templates</Link> · <Link href={`/domain/${slug}/forms`} title="Standalone form templates">Forms</Link></nav>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}><h1>Templates</h1>{canManageTemplates ? <span style={{ display: 'flex', gap: '.75rem' }}><Link href={`/domain/${slug}/document-types`} title="Templates hang off a Document Type — choose the Type first">Create template</Link><Link href={`/domain/${slug}/document-types`} title="Forms hang off a Document Type — choose the Type first">Create form</Link></span> : null}</div>
      <ul style={{ display: 'grid', gap: '.5rem', listStyle: 'none', padding: 0 }}>{templates.docs.map((template) => <li key={template.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '.75rem', border: '1px solid var(--tenant-border, #ddd)' }}><span><strong>{template.name}</strong> · {template.kind} · {template.active ? 'Active' : 'Inactive'}</span>{canManageTemplates ? <span style={{ display: 'flex', gap: '.5rem' }}>{template.kind === 'document' ? <Link href={`/domain/${slug}/templates/${template.id}/edit`}>Edit</Link> : <Link href={`/domain/${slug}/forms/${template.id}/edit`}>Edit</Link>}{template.active ? <form action={deactivateTemplateAction}><input type="hidden" name="domainSlug" value={slug} /><input type="hidden" name="templateId" value={template.id} /><button type="submit">Deactivate</button></form> : null}<form action={duplicateTemplateAction}><input type="hidden" name="domainSlug" value={slug} /><input type="hidden" name="templateId" value={template.id} /><button type="submit">Duplicate</button></form></span> : null}</li>)}</ul>
    </section>
  </TenantShell>
}
