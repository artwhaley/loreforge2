...
export default async function PersonWorkspacePage({ params, searchParams }: Props) {
  ...
  const roleFilter = (await searchParams)?.roleFilter === 'held' ? 'held' : 'assignable'
  ...
  return <TenantShell tenant={tenant} cssVars={themeTokensToCssVars(resolveThemeTokens(tenant))} role={contextRole} switcherTenants={domains} activeCharacter={activeCharacter}>
    <section className="personWorkspace">
      ...
      <section id="roles"><h2>Roles</h2><p>Roles belong to Departments. Assigning or removing one never edits Folder access.</p><p><a href={`${pageUrl}?roleFilter=held`}>Held roles</a> · <a href={`${pageUrl}?roleFilter=assignable`}>Roles I can assign</a></p><div role="tree" aria-label="Department Roles">{roleRows.map((item) => { const department = departmentById.get(String(relationId(item.subdomain))); const held = assignedRoleIds.has(String(item.id)); const parent = roleById.get(String(relationId(item.parentRole))); return <div key={item.id} role="treeitem" aria-level={parent ? 2 : 1} style={{ marginLeft: parent ? '1.5rem' : 0 }}><form action="/api/role-assignments" method="post"><input type="hidden" name="domainSlug" value={slug} /><input type="hidden" name="characterId" value={characterId} /><input type="hidden" name="roleId" value={item.id} /><input type="hidden" name="action" value={held ? 'remove' : 'add'} /><label><input type="checkbox" checked={held} readOnly onClick={(event) => { event.preventDefault(); (event.currentTarget.form as HTMLFormElement).submit() }} /> {item.name}</label><span> · {department?.name ?? 'Department'}{parent ? ` · reports to ${parent.name}` : ''}</span></form></div> })}</div>{roleRows.length === 0 ? <p>{roleFilter === 'held' ? 'No held Roles.' : 'No Roles available.'}</p> : null}</section>
      <section id="folder-access"><h2>Folder access</h2><p>Direct Character access is separate from Roles. These controls are provisional until the Phase 7 evaluator; they never mutate a Role assignment.</p><ul>{folderTree.map(({ folder, depth }) => { const readRule = ruleFor(Number(folder.id), ['read']); const writeRule = ruleFor(Number(folder.id), ['create_document', 'edit_document']); const readState = readRule?.effect ?? 'inherit'; const writeState = writeRule?.effect ?? 'inherit'; return <li key={folder.id} style={{ marginLeft: `${depth * 1.1}rem` }}><form action="/api/permission-rules" method="post"><input type="hidden" name="domainSlug" value={slug} /><input type="hidden" name="characterId" value={characterId} /><input type="hidden" name="folderId" value={folder.id} /><strong>{folder.name}</strong> <label>Read <select name="readState" defaultValue={readState}><option value="inherit">Inherited</option><option value="grant">Allow</option><option value="deny">Deny</option></select></label> <label>Write <select name="writeState" defaultValue={writeState}><option value="inherit">Inherited</option><option value="grant">Allow</option><option value="deny">Deny</option></select></label> <button type="submit">Save</button><small> · Read source: {readRule ? 'Direct Character rule' : 'Role / Folder default'} · Write source: {writeRule ? 'Direct Character rule' : 'Role / Folder default'}</small></form></li> })}</ul>{folderTree.length === 0 ? <p>No Folders are configured.</p> : null}</section>
