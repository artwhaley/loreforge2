...
export async function POST(request: Request) {
  ...
  if (action === 'delete') {
    ...
    try { await assertCanCreateRole(payload, { actor, domainId: domain.id, departmentId: idOf(role.subdomain) ?? 0 }) } catch { return NextResponse.redirect(new URL(destination, request.url), 303) }
  ...
  try { await assertCanCreateRole(payload, { actor, domainId: domain.id, departmentId: subdomainId }) } catch { return NextResponse.redirect(new URL(destination, request.url), 303) }
  try {
    assertRoleHierarchy(
