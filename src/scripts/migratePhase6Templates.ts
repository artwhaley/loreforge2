...
for (const domain of domains.docs) {
  ...
  if (!type) type = await payload.create({ collection: 'document-types', overrideAccess: true, data: { domain: domain.id, name: 'Plain Text', active: true, templateSelection: 'blank', allowBlank: true, allowTemplate: true, allowForm: false, defaultFilingPolicy: 'direct-file', templateFilingPolicy: 'inherit' } })
...
for (const form of forms.docs) {
  ...
  if (!type) type = await payload.create({ collection: 'document-types', overrideAccess: true, data: { domain: domain.id, name, active: true, templateSelection: 'form', allowBlank: true, allowTemplate: false, allowForm: true, defaultFilingPolicy: 'direct-file', templateFilingPolicy: 'inherit' } })
