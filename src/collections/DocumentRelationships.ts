...
/** Canonical document graph edges. Grouped is symmetric; Supersedes is newer -> older. */
export const DocumentRelationships: CollectionConfig = {
  slug: 'document-relationships',
  admin: { useAsTitle: 'kind', defaultColumns: ['kind', 'source', 'target', 'label', 'actorUser'] },
  timestamps: true,
  fields: [
    ...
    { name: 'kind', type: 'select', required: true, options: [{ label: 'Grouped', value: 'grouped' }, { label: 'Supersedes', value: 'supersedes' }] },
    { name: 'label', type: 'text', label: 'Grouped label' },
  ...
  hooks: {
    beforeChange: [async ({ data, originalDoc, req }) => {
      ...
      const label = String(data?.label ?? originalDoc?.label ?? '')
      ...
      if (source && target) {
        ...
        if (kind === 'grouped' && !label.trim()) throw new Error('Grouped relationships require a nonblank label.')
