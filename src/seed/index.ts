...
for (const [slug, domain] of Object.entries(domainsBySlug)) {
  ...
  const type = existing.docs[0] ?? await payload.create({ collection: 'document-types', data: { domain: domain.id, name: 'Plain Text', description: 'A freeform Markdown record.', active: true, templateSelection: 'blank', allowBlank: true, allowTemplate: true, allowForm: false, defaultFilingPolicy: 'direct-file', templateFilingPolicy: 'inherit' } })
...
for (const document of existingDocuments.docs) {
  ...
  await payload.update({ collection: 'documents', id: document.id, data: { documentType: document.documentType ?? typeId, sourceKind: document.sourceKind ?? sourceKind, lifecycle: document.lifecycle ?? 'filed', publicAccess: document.publicAccess ?? 'inherit' }, depth: 0, context: { authorizationChecked: true } })
}
