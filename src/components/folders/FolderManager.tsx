...
export type AdminFolderNode = { canManage?: boolean; id: number; name: string; systemManaged: boolean; effectiveRead?: { allowed: boolean; source: string }; effectiveWrite?: { allowed: boolean; source: string }; children: AdminFolderNode[] }
...
export function FolderManager({ domainSlug, folders, canManageRoot }: { domainSlug: string; folders: AdminFolderNode[]; canManageRoot: boolean }) {
  ...
  return <div className={styles.page} onClick={() => menu && setMenu(null)}>
    <div className={styles.toolbar}><div><h2>Folders</h2><p>Configure the document filing tree for this Domain.</p></div><button type="button" className={styles.button} disabled={!canManageRoot} onClick={() => { setTarget(null); setDialog('create') }}>New folder</button></div>
    ...
    {menu ? <div className={styles.contextMenu} style={{ left: menu.x, top: menu.y }} onClick={(event) => event.stopPropagation()} role="menu"><button type="button" disabled={menu.node.canManage === false} onClick={() => openDialog('create')}>New subfolder</button>{!menu.node.systemManaged ? <><button type="button" disabled={menu.node.canManage === false} onClick={() => openDialog('move')}>Move folder…</button><button type="button" disabled={menu.node.canManage === false} onClick={() => openDialog('delete')}>Delete folder</button></> : null}</div> : null}
