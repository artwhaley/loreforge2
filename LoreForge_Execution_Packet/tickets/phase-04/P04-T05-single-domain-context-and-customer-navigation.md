...
5. Refactor Domain shell navigation to stable `Home, About, Departments, Records`. Move all management links to a subordinate `Manage <Domain>` bar with `People, Roles, Templates & Forms, Customize`, showing only destinations authorized by the current interim boundary. Do not treat link hiding as authorization.
