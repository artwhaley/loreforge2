...
3. Implement one transactional merge matrix: re-point and exact-dedupe DomainCharacterContexts (source aliases retained; conflicting local fields block for manual resolution), DomainMemberships, RoleAssignments, direct Character PermissionRules including Folder overrides, and DocumentCharacterLinks; derive Department participation again from the merged RoleAssignments; transfer a sole Personal Domain and sole controlling User to survivor; retain historical audit/provenance actor IDs unchanged; mark source `merged` with `mergedInto` so old IDs resolve/redirect. Future Character-bearing collections must register an explicit merge adapter before release.
