...
## Objective
Add exceptional same-document sharing without conflating it with Copy, and make the three operations unmistakable in the UI.
...
- Copy and Share must use distinct language and confirmations.
...
4. Create a consolidated Document Actions surface with `Share existing document`, `Create independent copy`, and `Move canonical document`, each with concise explanatory text.
...
- Shared reader sees same Document ID/current version; copied recipient sees distinct ID.
...
## Manual acceptance
- Share a marriage license with another Character and verify same canonical record; then make an independent Personal-style copy simulation and compare IDs/history.
- Review action labels for ambiguity; a nontechnical tester must distinguish Share vs Copy without explanation.

## Guardrails / non-goals
- `Do not implement folder grants yet except schema compatibility.`
- `Do not make Share create a copy automatically.`
