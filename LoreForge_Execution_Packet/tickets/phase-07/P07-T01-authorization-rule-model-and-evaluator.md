...
## Required work
1. Complete PermissionRule exactly as frozen, preserving/migrating the P05 direct-Folder subset: Domain; principal type `User|Character|Role|DomainMembership` plus principal ID; resource type `Domain|Subdomain|Folder|Document` plus resource ID; capability; effect `grant|deny`; actor/audit fields. Do not add a separate `delegatable` flag: delegation is computed from the actor's own `manage_access` plus possession of the capability and scope.
...
3. Implement pure-ish evaluator that gathers applicable rules, Role ancestry, active Domain membership, and resource ancestry, then applies exact precedence. RoleAssignments contribute only their Role identity; they have no Folder scope.
4. Implement the complete capability vocabulary from the Architecture Contract, sourced from the single shared constant created in P05R-T04 (schema options == shared constant == contract list is pinned by regression): `read, create_document, edit_document, submit_document, file_document, approve_document, lock_document, unlock_document, delete_document, restore_document, share_document, export_document, manage_folders, manage_templates, manage_types_tags, manage_access, manage_members, manage_claims, manage_roles, assign_roles, assign_subordinates, manage_subdomain, manage_domain_appearance, manage_notices`. `move_document` and `copy_document` are retired product concepts (Copy/Move rejected) and must not appear. `share_document` remains reserved capability vocabulary only: its presence does not freeze or imply a Share workflow (CC-2026-09-03-04; the P05 Share prototype is deferred, see `references/P07-D01-DOCUMENT-SHARING-DECISION.md`). Later phases may add new named capabilities such as correspondence moderation without changing evaluator semantics.
5. Implement senior Role inheritance of subordinate Role default grants within the same Department. Do not introduce assignment-specific resource constraints.
...
- No SubdomainMembership principal or RoleAssignment Folder scope participates in evaluation.
...
- `Do not recreate scoped RoleAssignments inside PermissionRule matching.`
