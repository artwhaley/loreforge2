...
- `Character.kind` is exactly `player | npc | domain_admin | platform_admin`. `player` and `npc` use the ordinary Character → DomainMembership → Role → PermissionRule path and gain no authority from the controlling User's platform/owner/admin status. `domain_admin` is the system-provisioned administrator of exactly one Community Domain; `platform_admin` is the system-provisioned platform operator identity. Administrative kinds cannot receive Roles, DomainMemberships, or ordinary claims and are excluded from public/RP Character semantics.
- The acting identity is authoritative: the login User never silently contributes a stronger authority class after an acting identity is selected. Platform-only work requires acting `platform_admin`; Domain-administration work requires acting the matching `domain_admin`; ordinary work uses the ordinary evaluator.
- The one Domain selector contains Domains reachable through the acting identity: active controlled-Character participation, plus Domains for which the User holds a provisioned administrative identity. The UI may group them as `Participating` and `Managed`, but there is no second Administration selector.
- For a selected Domain, the Character selector offers the User's provisioned administrative identities when eligible (Platform Admin `Administrator` platform-wide; `Administrator of <Domain>` only for a Domain the User owns) plus active ordinary Characters controlled by the User with active membership in that Domain. A `domain-character-context` alias never makes a Character selectable. A selected Domain with no eligible identity is valid for public browsing and displays `No participating Character` honestly.
- Domain selection never silently changes the acting Character. If the prior Character is invalid in the newly selected Domain, clear the Character selection while preserving the selected Domain. Character-scoped reads/actions require a server-validated `(User, Character, Domain)` tuple; administrative actions validate the acting identity's kind and scope.
...
- `owner`, `Domain Admin`, and `Domain owner` are synonyms for that one person. The Owner User is provisioned exactly one `domain_admin` Character (`Administrator of <Domain>`). The legacy multi-operational-admin `domain-admins` model is retired as an authority source; its rows never grant authority after the P07X corrective.
...
- Document Type is the primary ordinary record authorization unit: Roles grant record capabilities (read/create/edit/delete/submit/approve/…) on Document Types. Folder permissions remain real but organize/restrict that work: a Folder grant never manufactures a missing Type capability, and effective Folder/ancestor denies may narrow a Type/Document grant. Document-specific sharing/grants/denies remain supported as direct same-record exceptions.
...
- Platform administration is an acting identity: a platform-admin-eligible User is provisioned exactly one `platform_admin` Character (`Administrator`). Platform-only operations require acting as that identity, which grants no ordinary Domain record mutation authority.
